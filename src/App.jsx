...
export default function App(){
  ...
  const resetData=async()=>{if(!confirm("⚠️ ยืนยันลบข้อมูลทั้งหมด?"))return;if(!confirm("คุณแน่ใจหรือไม่? ข้อมูลทั้งหมดจะถูกลบถาวร"))return;setProjects([]);setPriceSets([]);setProjectData([]);setActiveProject(null);setActivePriceSet(null);alert("ลบข้อมูลสำเร็จ");};
...
function Tab1({projects,setProjects,activeProject,setActiveProject,setTab}){
  ...
  const del=(id)=>{if(confirm("ยืนยันลบโครงการนี้?")){setProjects(p=>p.filter(x=>x.id!==id));if(activeProject===id)setActiveProject(null);}};
...
function Tab2({priceSets,setPriceSets,activePriceSet,setActivePriceSet}){
  ...
  const del=(id)=>{if(confirm("ยืนยันลบ?")){setPriceSets(p=>p.filter(x=>x.id!==id));if(activePriceSet===id)setActivePriceSet(null);}};
...
function Tab3({activeProject,activePriceSet,projectData,setProjectData,priceSets,projects}){
  ...
  const del=(id)=>{if(confirm("ยืนยันลบ?"))setProjectData(p=>p.filter(x=>x.id!==id));};
